...
$thetime = time();
if ( ($timeleft < 1 ) && $board_config['lottery_start'])
{
	$sql = "SELECT *
		FROM " . LOTTERY_TABLE;
...
//         }
		...
		$sql = "INSERT INTO " . LOTTERY_HISTORY_TABLE . "
			...
			VALUES ('{$row['user_id']}', '$pool', '$currency_name', '$thetime')";
		if ( !($db->sql_query($sql)) )
		{
...
if ( empty($action) )
{
	...
	$title = $lang['lottery_information'] . $board_config['lottery_name'];
	...
	{
		if ( $tickbuy && $board_config['lottery_start'] )
		{
			#
	...
	$template->assign_vars(array(
		...
		'L_INFO_TITLE' => $lang['lottery_information'] . ' ' . $board_config['lottery_name'],
