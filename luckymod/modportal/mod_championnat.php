...
$template_mod->set_filenames(array(
   'body' => $phpbb_root_path . '/templates/' . $theme['template_name'] . '/modportal/mod_championnat.tpl')
);
...
if ($arcade_config['championnat_use'] == 1)
{
	...
	if ($liste_userid!='')
	{
		$sql = "SELECT user_id,username,user_level,user_whosonline_color FROM " . USERS_TABLE . " WHERE user_id IN ($liste_userid)";
		if( !$result = $db->sql_query($sql) )
		{
	...
	$template_mod->assign_block_vars( 'championnat_row', array( 
		'CLASS' => $class,
...
if ($arcade_config['championnat_use'] == 1)
{
	$template_mod->assign_block_vars( 'championnat_use', array());
