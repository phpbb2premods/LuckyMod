...
$sql = str_replace(', u2.user_id as id2', ', u2.user_id as id2, u2.user_level as level2, u2.user_whosonline_color as erc_color2', $sql);
//-- fin mod : totally erc -----------------------------------------------------	
if ( !($result = $db->sql_query($sql)) )
{
...
if ($attachments)
{
		...
		$sql2 = "SELECT f.*, p.post_time, p.post_username, u.username, u.user_level, u.user_whosonline_color, u.user_id, t.topic_id, t.topic_title
			FROM ((( " . FORUMS_TABLE . " f
...
//-- add 
            ...
            { 
               case ADMIN: 
                  $username_last_post = '<b>' . $value['username'] . '</b>'; 
            ...
            if ( $value['user_whosonline_color'] && $board_config['allow_totally_erc'] ) 
            { 
               $username_last_post = '<b>' . $value['username'] . '</b>'; 
               $style_color = 'style="color:' . $id_color[$value[user_whosonline_color]] . '"'; 
            } 
            else if ( $user_group_color[$value['user_id']] && $board_config['allow_totally_erc'] ) 
            { 
               $username_last_post = '<b>' . $value['username'] . '</b>'; 
               $style_color = 'style="color:' . $user_group_color[$value['user_id']] . '"'; 
            }
					...
					if ( $value['forum_last_post_id'] )
					{
						...
						$last_post .= '<br /> '; 
						$last_post .= ( $value['user_id'] == ANONYMOUS ) ? ( ($value['post_username'] != '' ) ? $value['post_username'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $value['user_id']) . '"' . $style_color . '>' . $username_last_post . '</a>&nbsp;';
...
MOD-*/
//-- add
		$topic_author_color = $erc->get_colors($topic_rowset[$i]);
...
MOD-*/
//-- add
		$last_post_author_color = $erc->get_colors($topic_rowset[$i], '', false, 'id2', 'erc_color2', 'level2');
