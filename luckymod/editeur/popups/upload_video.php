  ...
  <table border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse: collapse\" bordercolor=\"#111111\" width=\"300\" id=\"AutoNumber1\">
    ...
    <tr>
      <td bgcolor=\"#D4D0C8\"><font size=\"2\">Les restrictions suivantes s'appliquent:</font><ul type=\"square\">
        ...
        if (($extensions_video == "") or ($extensions_video == " ") or ($ext_count_video == "0") or ($ext_count_video == "") or ($limit_ext_video != "yes") or ($limit_ext_video == "")) {
           echo "";
        } else {
        $ext_count_video = $ext_count_video+1;
        for($counter=0; $counter<$ext_count_video; $counter++) {
            echo "&nbsp; $extensions_video[$counter]";
        }
        }
        if (($limit_size_video == "") or ($size_limit-video != "yes")) {
            $limit_size_video = "any size";
        } else {
            $limit_size_video .= " bytes";
        }
        echo"</b></font></li>
...
<button name=\"submit\" type=\"submit\" style=\"font-family: v; font-size: 10pt; color: #5E6A7B; border: 1px solid #5E6A7B; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1\"><b>envoyer</b></button>
    ...
    <tr>
      ...
      <a href=\"insert_video.php\" style=\"text-decoration: none\"><font color=\"#FFFFFF\"><b>Fermer</b></font></a>
      </font></td>
    </tr>
  </table>
  </center>
...
if(file_exists("$absolute_path_video/$file_name")) {
$endresult = "<font size=\"2\"><font color=\"#000000\">Le fichier existe déja</font>";
} else {
if (($size_limit-video == "yes") && ($limit_size_video < $file_size)) {
$endresult = "<font size=\"2\"><font color=\"#000000\">Le fichier est trop gros</font>";
} else {
$ext = strrchr($file_name,'.');
if (($limit_ext_video == "yes") && (!in_array($ext,$extensions_video))) {
$endresult = "<font size=\"2\">le fichier n'est pas un .swf</font>";
}else{
@copy($file, "$absolute_path_video/$file_name") or $endresult = "<font size=\"2\">le fichier a été up-loader</font>";
}
}
}
}
