...
switch( $mode )
{
	...
	case 'split':
		...
		else
		{
			...
			$sql = "SELECT u.username, u.user_id, u.user_level, u.user_whosonline_color, p.*, pt.post_text, pt.bbcode_uid, pt.post_subject, p.post_username
...
MOD-*/
//-- add
					'USERNAME' => ( $id == ANONYMOUS ) ? $lang['Guest'] : $erc->get_colors($row, $row['username']),
//-- fin mod : totally erc -----------------------------------------------------
					'POSTS' => $row['postings'] . ' ' . ( ( $row['postings'] == 1 ) ? $lang['Post'] : $lang['Posts'] ),
	...
	default:
		...
		$sql = "SELECT t.*, u.username, p.post_username, p.post_time
			...
			//-- mod : totally erc ---------------------------------------------------------
//-- add
		$sql = str_replace(', u.username', ', u.username, u.user_id, u.user_level, u.user_whosonline_color, p.post_username', $sql);
		$sql = str_replace('AND t.topic_poster = u.user_id', 'AND p.poster_id = u.user_id', $sql);
//-- fin mod : totally erc -----------------------------------------------------
		if ( !($result = $db->sql_query($sql)) )
		{
...
//-- add
			...
			$last_post_author = ($row['user_id'] == ANONYMOUS) ? (($row['post_username'] != '') ? $row['post_username'] : $lang['Guest']) : '<a href="' . append_sid('profile.' . $phpEx . '?mode=viewprofile&amp;' . POST_USERS_URL . '='  . $row['user_id']) . '"' . $style_color . '>' . $row['username'] . '</a>';
