...
$new_bbcode_uid = make_bbcode_uid();
if( !empty($mode) ) 
{
	switch($mode)
	{
...
// Added by Attached Forums MOD
   ...
   // End Added by Attached Forums MOD
				...
				'DESCRIPTION' => ( empty($row['bbcode_uid']) ) ? $forumdesc : preg_replace('/\:(([a-z0-9]:)?)' . $row['bbcode_uid'] . '/s', '', $forumdesc))
			);
		...
		$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, attached_forum_id, forum_desc, forum_order, forum_status, prune_enable" . $field_sql . ", bbcode_uid)
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " . intval($HTTP_POST_VARS[POST_CAT_URL]) .  ", " . intval($HTTP_POST_VARS['attached_forum_id']) . ", '" . str_replace("\'", "''", ( $board_config['allow_bbcode'] ) ? bbencode_first_pass($HTTP_POST_VARS['forumdesc'], $new_bbcode_uid) : $HTTP_POST_VARS['forumdesc']) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", " . intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ", '$new_bbcode_uid')";
   // End Added by Attached Forums MOD
		...
		case 'modforum':
			...
			$sql = "UPDATE " . FORUMS_TABLE . "
				SET forum_name = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', cat_id = " . intval($HTTP_POST_VARS[POST_CAT_URL]) .", attached_forum_id = " . intval($HTTP_POST_VARS['attached_forum_id']) . ", forum_desc = '" . str_replace("\'", "''", ( $board_config['allow_bbcode'] ) ? bbencode_first_pass($HTTP_POST_VARS['forumdesc'], $new_bbcode_uid) : $HTTP_POST_VARS['forumdesc']) . "', forum_status = " . intval($HTTP_POST_VARS['forumstatus']) . ", prune_enable = " . intval($HTTP_POST_VARS['prune_enable']) . ", bbcode_uid = '$new_bbcode_uid' 
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id)
			{
				...
				if ($do_template)
				{
					$template->assign_block_vars("catrow.forumrow",	array(
						...
						'FORUM_DESC' => ( $board_config['allow_bbcode'] ) ? make_clickable(bbencode_second_pass(( $board_config['allow_smilies'] ) ? str_replace('src="'.$board_config['smilies_path'], 'src="../'.$board_config['smilies_path'], smilies_pass($forum_rows[$j]['forum_desc'])) : $forum_rows[$j]['forum_desc'], $forum_rows[$j]['bbcode_uid'])) : preg_replace('/\:[0-9a-z\:]+\]/si', ']', ( $board_config['allow_smilies'] ) ? str_replace('src="'.$board_config['smilies_path'], 'src="../'.$board_config['smilies_path'], smilies_pass($forum_rows[$j]['forum_desc'])) : $forum_rows[$j]['forum_desc']),
						'ROW_COLOR' => $row_color,
