...
//-- add
				...
				$erc->update_colors(intval($rows[$i]['user_id']));
//-- fin mod : totally erc -----------------------------------------------------
			}
		}
...
//-- add
					...
					$erc->update_colors($group_info['group_moderator']);
//-- fin mod : totally erc -----------------------------------------------------
					$sql = "DELETE FROM " . USER_GROUP_TABLE . "
						WHERE user_id = " . $group_info['group_moderator'] . " 
							AND group_id = " . $group_id;
...
//-- add
			if ($group_info['group_color'] != $group_color)
			{
				...
				if (!$result = $db->sql_query($sql))
				{
					message_die(GENERAL_ERROR, 'Failed to obtain group membership', '', __LINE__, __FILE__, $sql);
				}
				...
				if (!empty($list_ids))
				{
					...
					$erc->update_colors($result_ids, $group_color, true);
				}
...
//-- add
			...
			{
				$erc->update_colors(intval($group_moderator), $group_color);
