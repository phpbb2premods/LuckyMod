		...
		if( $HTTP_POST_VARS['deleteuser'] && ( $userdata['user_id'] != $user_id ) )
{
			...
			// mod ARCADE : Mise à jour des arcades impliquant l'utilisateur supprimé 
   $sql = "DELETE FROM " . SCORES_TABLE . " 
      WHERE user_id = $user_id"; 
   if ( !$db->sql_query($sql) ) 
   { 
      message_die(GENERAL_ERROR, 'Impossible de supprimer l\'utilisateur de la table des scores', '', __LINE__, __FILE__, $sql); 
   } 
   // si jamais l'utilisateur supprimé avait le highscore, on recherche le nouveau highscore 
   ...
   $sql = "DELETE FROM " . ARCADE_CHEATER_TABLE . "
			...
			if ( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete user from cheater_list table', '', __LINE__, __FILE__, $sql);
			} 
...
// End add - Birthday MOD
		...
		if( !$error )
		{
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_whosonline_color='$whosonline_rank', user_birthday='$birthday', user_next_birthday_greeting=$next_birthday_greeting, user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_rank = $user_rank, user_miniland = '$miniland', user_mood = '$mood', user_gender = '$gender'" . $avatar_sql . "
				WHERE user_id = $user_id";
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$miniland = $this_userdata['user_miniland'];
		// Start add - Mood MOD
		$mood = $this_userdata['user_mood'];
		// End add - Mood MOD
// Start add - Gender MOD
...
$s_hidden_fields .= '<input type="hidden" name="miniland" value="' . $miniland . '" />';
			// Start add - Mood MOD
			$s_hidden_fields .= '<input type="hidden" name="mood" value="' . $mood . '" />';
			// End add - Mood MOD
// Start add - Gender MOD
...
// Start add - Mood MOD
		...
		{ 
			...
			default:$mood_no_specify_checked="checked=\"checked\""; 
		}
		// End add - Mood MOD
		switch ($miniland) 
{ 
   ...
   default:$miniland_no_specify_checked="checked=\"checked\""; 
}
		//
